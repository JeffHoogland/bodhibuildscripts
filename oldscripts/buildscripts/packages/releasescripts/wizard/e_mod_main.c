...
e_modapi_shutdown(E_Module *m __UNUSED__)
{
   ...
   if (!default_e_theme) return 1;

   ct = e_theme_config_get("theme");
   if ((ct) && (!strcmp(ct->file, default_e_theme))) return 1;

   e_config->init_default_theme = eina_stringshare_add(default_e_theme);
   e_theme_config_set("theme", default_e_theme);
   e_config_save();
   a = e_action_find("restart");
   if ((a) && (a->func.go)) a->func.go(NULL, NULL);
   eina_stringshare_del(default_e_theme);
// FIXME: wrong place   
